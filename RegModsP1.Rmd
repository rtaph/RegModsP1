...
A number of exploratory charts are presented in the appendicess. From initial inspection, there appears to be strong correlation between mpg and the `cyl`, `disp`, `wt` variables (the regressionn lines seem to fit the data well). 
...
```
A review of the model summary (see appendices) reveals that it is statistically significant (*p* < .01).
...
The multivariate model improves our regression significantly compared to the initial model (RSS of `r round(anova(aicFormula, mod1)$RSS[1],0)` vs `r round(anova(aicFormula, mod1)$RSS[2],0)`). This difference is statistically signficant (*p* < .01).
#### Model 2
